...
var bitapRegexSearch = __webpack_require__(7);
var bitapSearch = __webpack_require__(9);
...
var deepValue = function deepValue(obj, path, list) {
  ...
  var value = obj.get(firstSegment);
  ...
  if (value !== null && value !== undefined) {
    if (!remaining && (typeof value === 'string' || typeof value === 'number')) {
      ...
      return value.reduce(function (accumulation, item) {
        return deepValue(item, remaining, accumulation);
      }, list);
    } else if (remaining) {
...
var bitapScore = __webpack_require__(8);
...
var isArray = __webpack_require__(0);
var deepValue = __webpack_require__(4);
...
var Fuse = function () {
  function Fuse(list, _ref) {
    ...
    this.weights = typeof list.first() !== 'string' ? this.options.keys.reduce(function (weights, key) {
  ...
  _createClass(Fuse, [{
    ...
    value: function search(pattern) {
      ...
      var _options = this.options,
          tokenize = _options.tokenize,
          tokenSeparator = _options.tokenSeparator,
          shouldSort = _options.shouldSort;

      var tokenSearchers = tokenize ? pattern.split(tokenSeparator).map(function (token) {
        return _this._createSearcher(token);
      }) : [];
    ...
    key: '_createSearcher',
    value: function _createSearcher(pattern) {
      return new Bitap(pattern, this.options);
    }
  }, {
    key: '_searchCollectionOfStrings',
    value: function _searchCollectionOfStrings(tokenSearchers, fullSearcher) {
      var _this2 = this;

      return Seq(this.list).filter(function (value) {
        return typeof value === 'string';
      }).map(function (value) {
        return _this2._analyzeString({
          key: '',
          value: value,
        ...
        return item.getIn(['output', 0, 'isMatch']);
      });
    }
  }, {
    key: '_searchCollectionOfMaps',
    value: function _searchCollectionOfMaps(tokenSearchers, fullSearcher) {
      var _this3 = this;

      return Seq(this.list).map(function (item) {
        return Seq(_this3.options.keys).map(function (key) {
          return typeof key !== 'string' ? key.name : key;
        }).map(function (key) {
          return [key, _this3.options.getFn(item, key)];
          ...
          return [key, typeof value === 'string' ? List.of(value) : value];
        }).map(function (_ref6) {
          var _ref7 = _slicedToArray(_ref6, 2),
              key = _ref7[0],
              values = _ref7[1];

          return Seq(values).map(function (value) {
            return _this3._analyzeString({ key: key, value: value, tokenSearchers: tokenSearchers, fullSearcher: fullSearcher });
          }).filter(function (result) {
            return result.get('isMatch');
          });
        }).toList().reduce(function (keysResults, keyResults) {
          return keysResults.withMutations(function (mutableKeysResults) {
            return mutableKeysResults.update('output', function (output) {
              return output.concat(keyResults);
            }).update('matchedTokens', function (matchedTokens) {
              return Seq(keyResults).reduce(function (accumulation, result) {
                return accumulation.union(result.get('matchedTokens'));
              }, matchedTokens);
            });
          });
        }, Map({ item: item, output: List(), matchedTokens: Set() }));
      }).filterNot(function (result) {
        return result.get('output').isEmpty();
      });
    }
  }, {
    key: '_search',
    value: function _search() {
      ...
      var list = this.list;
      var results = typeof list.first() === 'string' ? this._searchCollectionOfStrings(tokenSearchers, fullSearcher) : this._searchCollectionOfMaps(tokenSearchers, fullSearcher);

      if (tokenize && matchAllTokens) {
        return results.filter(function (item) {
          return item.get('matchedTokens').size === tokenSearchers.length;
        });
      }
    ...
    key: '_analyzeString',
    value: function _analyzeString(_ref8) {
      var _this4 = this;

      var key = _ref8.key,
          _ref8$arrayIndex = _ref8.arrayIndex,
          arrayIndex = _ref8$arrayIndex === undefined ? -1 : _ref8$arrayIndex,
          value = _ref8.value,
          _ref8$tokenSearchers = _ref8.tokenSearchers,
          tokenSearchers = _ref8$tokenSearchers === undefined ? [] : _ref8$tokenSearchers,
          fullSearcher = _ref8.fullSearcher;
      ...
      if (this.options.tokenize) {
        ...
        var scores = tokenSearchers.reduce(function (accumulation, tokenSearcher) {
          _this4._log('\nPattern: "' + tokenSearcher.pattern + '"');

          var tokenScores = words.reduce(function (wordScores, word) {
            ...
            if (!_this4.options.matchAllTokens) {
      ...
      var finalScore = averageScore > -1 ? average(List.of(mainSearchResult.score, averageScore)) : mainSearchResult.score;
    ...
    value: function _computeScore(results) {
      ...
      return results.map(function (result) {
        return result.withMutations(function (mutableResult) {
          ...
          var computedScores = result.get('output').reduce(function (scores, output, index) {
            var weight = weights ? weights.getIn([output.get('key'), 'weight']) : 1;
            var score = weight === 1 ? output.get('score') : output.get('score') || 0.001;
            var nScore = score * weight;

            if (weight !== 1) {
              return scores.update('bestScore', function (bestScore) {
                return Math.min(bestScore, nScore);
              });
            } else {
              mutableResult.setIn(['output', index, 'nScore'], nScore);
              return scores.update('totalScore', function (totalScore) {
                return totalScore + nScore;
              });
            }
          }, Map({
            totalScore: 0,
    ...
    value: function _format(results) {
      ...
      if (includeMatches) {
        transformers.push(function (result, data) {
          return data.set('matches', result.get('output').reduce(function (matches, item) {
      ...
      if (includeScore) {
        transformers.push(function (result, data) {
          return data.set('score', result.get('score'));
        });
      }

      return results.map(function (result) {
        var item = id ? getFn(result.get('item'), id).first() : result.get('item');

        if (!transformers.length) {
          return item;
        }

        return transformers.reduce(function (data, xf) {
          return xf(result, data);
        }, Map({ item: item }));
      });
    }
  }, {
